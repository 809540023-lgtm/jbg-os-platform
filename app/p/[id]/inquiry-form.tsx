...
export function InquiryForm({ productId }: { productId: string }) {
  ...
  return (
    <section className="mt-6 rounded-xl border border-line bg-panel/40 p-5">
      <h2 className="text-sm font-semibold text-slate-800">詢問這台設備</h2>
      <p className="mt-1 text-xs text-slate-500">AI 客服即時回覆規格/現貨/地區；議價與到府安裝由專人處理。</p>
      ...
      {state.message && (
        <div
          className={`mt-3 rounded-md border px-3 py-2 text-sm ${
            state.ok ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-700" : "border-red-500/30 bg-red-500/10 text-red-700"
          }`}
        >
          <p>{state.message}</p>
          {state.reply && <p className="mt-2 whitespace-pre-line text-slate-800">{state.reply}</p>}
          {state.pendingHuman && <p className="mt-1 text-xs text-emerald-700/70">（此類問題由專人回覆，確保報價與承諾準確）</p>}
        </div>
      )}
    </section>
  );
